# openAPI schema part0 for networksecurity.cnrm.cloud.google.com.v1beta1.NetworkSecurityServerTLSPolicy
  ...
  spec:
    properties:
      ...
      location:
        ...
        type: string
      mtlsPolicy:
        description: Optional. Defines a mechanism to provision peer validation certificates
          for peer to peer authentication (Mutual TLS - mTLS). If not specified, client
          certificate will not be requested. The connection is treated as TLS and
          not mTLS. If allow_open and mtls_policy are set, server allows both plain
                  ...
                  properties:
                    pluginInstance:
                      description: Required. Plugin instance name, used to locate
                        and load CertificateProvider instance configuration. Set to
      ...
      resourceID:
        ...
        type: string
      serverCertificate:
        description: Optional. Defines a mechanism to provision server identity (public
          and private keys). Cannot be combined with allow_open as a permissive mode
          that allows both plain text and TLS is not supported.
        properties:
          certificateProviderInstance:
